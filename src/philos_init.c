...
/*   Updated: 2025/05/13 16:48:13 by laufarin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mem_hilos(pthread_t **threads, t_resources *resources)
{
	...
	*threads = malloc(sizeof(pthread_t) * resources->number_of_philosophers);
	resources->philos = malloc(sizeof(t_philosopher)
	...
	while (i < resources->number_of_philosophers)
	{
		...
		(resources->philos)[i].left_fork = &resources->forks[i];
		(resources->philos)[i].right_fork = &resources->forks[(i + 1)
			% resources->number_of_philosophers];
		i++;
	}
	return (0);
